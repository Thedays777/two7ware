...
function setupMenu() {
    ...
    function setupWatermark() {
        ...
        watermark.innerHTML = `<span style="text-shadow: -1px 0.5px 0 #72ff72, -2px 0px 0 #2f672e;">KC</span> <span style="color:gray; padding-left:2px; font-family: Arial, sans-serif; font-size:10px">${ver}</span>`;
    ...
    function setupDropdown() {
        ...
        let featuresList = [
            ...
            [{ name: 'autoAnswer', type: 'checkbox', variable: 'features.autoAnswer', dependent: 'autoAnswerDelay,nextRecomendation,repeatQuestion', labeled: true, label: 'Auto Answer' },
             { name: 'autoAnswerDelay', className: 'autoAnswerDelay', type: 'range', variable: 'features.autoAnswerDelay', attributes: 'min="1" max="3" value="1"', labeled: false }],
        ...
        if (!device.apple) {
            featuresList.push(
                ...
                [{ name: 'Custom icon', type: 'nonInput' }, { name: 'customPfp', type: 'text', variable: 'featureConfigs.customPfp', attributes: 'autocomplete="off"' }]
            );
        }
    ...
    function setupStatusPanel() {
        ...
        const update = async () => statsPanel.innerHTML = `
            <span style="text-shadow: -1px 0.5px 0 #p, -2px 0px 0 #2f672e;">KHANCRIPT</span>
    ...
    function loadWidgetBot() {
        ...
        const script = Object.assign(document.createElement('script'), {
            ...
            async: true,
            onload: () => {
                const discEmbed = new Crate({ server: '1189230891810172958', channel: '1189278170009047061',
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ "🔥 Feito para você", "🤍 Made by Two7 Leaks", "☄️ By Two7 Leaks" ];
    ...
    function changeBannerText() {
        const phrases = [ "[🔥] Irmandade acima de tudo.", "[🔥] Two7 no topo.", "[👋] Thedays manda um salve!", "[🗡️] Dente por dente, olho por olho.", "[🌿] Feito para você", "[🌿] Os antigos nos saudam" ];
        setInterval(() => { 
...
/* Inject */
if (!/^https?:\/\/pt\.khanacademy\.org/.test(window.location.href)) { alert("❌ KhanCript Failed to Injected!\n\nVocê precisa executar o KhanCript no site do Khan Academy! (https://pt.khanacademy.org/)"); window.location.href = "https://pt.khanacademy.org/";};
...
.then(async () => {
    sendToast("👿 KhanCript injetado com sucesso!");
